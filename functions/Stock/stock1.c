...
void search_stock_by_stock_ID(){
  ...
  printf("%s\t %s\t\t %s\t %s\t\t %s\n", "Stock_ID", "Product_ID", "Quantity","Warehouse_ID","Expire_Date");
  printf("------------------------------------------------------------------------------------------------\n");
  while (fgets(search_st_by_id, sizeof(search_st_by_id), productTable) != NULL) {
      sscanf(search_st_by_id, "%d,%d,%d,%d,%d",&st_count,&product.id,&product.quantity,&product.warehouse_id,&product.expire_date);
